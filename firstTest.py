...
csv_writer.writerow(["nose X", "nose Y", "left eye X", "left eye Y", "right eye X", "right eye Y", \
                     "left eye bottom X", "left eye bottom Y", "right eye bottom X", "right eye bottom Y", \
                     "left eye top X", "left eye top Y", "right eye top X", "right eye top Y", \
                     "left eye inner X", "left eye inner Y", "right eye inner X", "right eye inner Y", \
                     "left eye outer X", "left eye outer Y", "right eye outer X", "right eye outer Y", \
                     "left_pupil X", "left_pupil Y", "right_pupil X", "right_pupil Y", "Yaw", "Pitch", "Roll", "Face Size", \
                        "target X normalized", "target Y normalized"])
...
while cap.isOpened():
    ...
    calib_img = etk.gen_red_circle_on_white_bg(size=(SCREEN_WIDTH, SCREEN_HEIGHT),\
                                                circle_radius=20, circle_position=(int(cross_position_x), int(cross_position_y)))
    
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            face_orientation.append(etk.get_face_orientation(face_landmarks.landmark, CHIN_LANDMARK, NOSE_LANDMARK, LEFT_EYE_OUTER, RIGHT_EYE_OUTER, LEFT_MOUTH, RIGHT_MOUTH))
            face_size = etk.get_face_size(face_landmarks.landmark, CHIN_LANDMARK, NOSE_LANDMARK, LEFT_EYE_OUTER, RIGHT_EYE_OUTER)
            #fit ellipse to left eye points
            left_eye_ellipse, left_eye_points = etk.fit_ellipse_to_eye(face_landmarks.landmark, LEFT_EYE_LANDMARKS, h, w)
            right_eye_ellipse, right_eye_points = etk.fit_ellipse_to_eye(face_landmarks.landmark, RIGHT_EYE_LANDMARKS, h, w)
           
            if left_eye_ellipse is not None and right_eye_ellipse is not None:
                # Crée un masque pour les yeux
                left_eye_img, left_eye_mask = etk.extract_eye_region(frame, left_eye_ellipse)
                right_eye_img, right_eye_mask = etk.extract_eye_region(frame, right_eye_ellipse)
                ...
                left_pupil = etk.extract_pupil3(left_eye_img, left_eye_mask[y:y+h_box, x:x+w_box])
                right_pupil = etk.extract_pupil3(right_eye_img, right_eye_mask[y:y+h_box, x:x+w_box])

                #get the centroid of each pupil
                M_left = cv2.moments(left_pupil)
                M_right = cv2.moments(right_pupil)

                left_bufferX.add(int(M_left["m10"] / M_left["m00"]) if M_left["m00"] != 0 else 0)
                left_bufferY.add(int(M_left["m01"] / M_left["m00"]) if M_left["m00"] != 0 else 0)

                right_bufferX.add(int(M_right["m10"] / M_right["m00"]) if M_right["m00"] != 0 else 0)
                right_bufferY.add(int(M_right["m01"] / M_right["m00"]) if M_right["m00"] != 0 else 0)
                ...
                if M_right_eye["m00"] != 0:
                    ...
                    cv2.circle(frame, (cX_right_eye, cY_right_eye), 2, (255, 255, 0), -1)

        for i, (yaw, pitch, roll) in enumerate(face_orientation):
            cv2.putText(frame, f"Yaw: {yaw:.2f}", (10, 30 + i * 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Pitch: {pitch:.2f}", (10, 60 + i * 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Roll: {roll:.2f}", (10, 90 + i * 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Face Size: {face_size:.4f}", (10, 120 + i * 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            cv2.circle(frame, (int(leftEyeBottomX * w), int(leftEyeBottomY * h)), 2, (255, 0, 0), -1)
            cv2.circle(frame, (int(rightEyeBottomX * w), int(rightEyeBottomY * h)), 2, (0, 0, 255), -1)
            ...
            csv_writer.writerow([noseX, noseY, leftEyeX, leftEyeY, rightEyeX, rightEyeY, \
                                leftEyeBottomX, leftEyeBottomY, rightEyeBottomX, rightEyeBottomY, \
                                leftEyeTopX, leftEyeTopY, rightEyeTopX, rightEyeTopY, \
                                leftEyeInnerX, leftEyeInnerY, rightEyeInnerX, rightEyeInnerY, \
                                leftEyeOuterX, leftEyeOuterY, rightEyeOuterX, rightEyeOuterY, \
                                leftPupilX, leftPupilY, rightPupilX, rightPupilY, yaw, pitch, roll, face_size, \
                                (cross_position_x) / SCREEN_WIDTH, (cross_position_y) / SCREEN_HEIGHT])
            
        # Agrandit l'image 4x
        frame_big = cv2.resize(frame, (0, 0), fx=1.6, fy=1.6, interpolation=cv2.INTER_NEAREST)
        eye_cropped_big = cv2.resize(eyes_cropped, (0, 0), fx=10, fy=10, interpolation=cv2.INTER_NEAREST)
        ...
        input_tensor = torch.tensor([[noseX, noseY, leftEyeX, leftEyeY, rightEyeX, rightEyeY, \
                                                leftEyeBottomX, leftEyeBottomY, rightEyeBottomX, rightEyeBottomY, \
                                                leftEyeTopX, leftEyeTopY, rightEyeTopX, rightEyeTopY, \
                                                leftEyeInnerX, leftEyeInnerY, rightEyeInnerX, rightEyeInnerY, \
                                                leftEyeOuterX, leftEyeOuterY, rightEyeOuterX, rightEyeOuterY, \
                                            leftPupilX, leftPupilY, rightPupilX, rightPupilY, \
                                                yaw, pitch, roll, face_size]], dtype=torch.float32)
        with torch.no_grad():
