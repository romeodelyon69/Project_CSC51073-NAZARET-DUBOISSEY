...
def extract_pupil3(eye_region, eye_mask):
    ...
    iris_gray_show = cv2.resize(iris_gray, (0, 0), fx=10, fy=10, interpolation=cv2.INTER_NEAREST)
...
def get_face_orientation(landmarks, chin_idx, nose_idx, left_eye_idx, right_eye_idx, left_mouth_idx, right_mouth_idx):
...
def gen_red_circle_on_white_bg(size=(1920, 1080), circle_radius=20, circle_position=(0, 0)):
...
def compute_gaze_physically(right_eye_yaw, left_eye_yaw, right_eye_pitch, left_eye_pitch, face_yaw, face_pitch, face_X, face_Y, face_Z):
